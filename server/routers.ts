...
export const appRouter = router({
  ...
  auth: router({
    ...
    register: publicProcedure.input(profileSchema).mutation(async ({ input, ctx }) => {
      ...
      const [created] = await db.insert(users).values({ openId: `local_${randomBytes(16).toString("hex")}`, name: input.name, email, cpf: input.cpf, birthDate: input.birthDate, passwordHash: await hashPassword(input.password), role: existing.length ? "student" : "global_admin" }).$returningId();
